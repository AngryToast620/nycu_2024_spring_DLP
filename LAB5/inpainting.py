    ...
    def inpainting(self,image,mask_b,i): #MaskGIT inference
        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                ...
                z_indices_predict, mask_bc, z_indices = self.model.inpainting(z_indices, mask_b, mask_bc, self.total_iter, step)
                ...
                # print(self.model.vqgan.decoder)
                decoded_img=self.model.vqgan.decode(z_q)
                # print(decoded_img[0])
                dec_img_ori=(decoded_img[0]*std)+mean
                imga[step+1]=dec_img_ori #get decoded image
                # imga.append(dec_img_ori)

            ##decoded image of the sweet spot only, the test_results folder path will be the --predicted-path for fid score calculation
                results_path = os.path.join("test_results", "test", f'{step}', f"image_{i:03d}.png")
                if os.path.exists(results_path):
                    os.remove(results_path)
                vutils.save_image(dec_img_ori, results_path, nrow=1) 

            #demo score 
            mask_path = os.path.join("mask_scheduling", "cos1", f"test_{i}.png")
            if os.path.exists(mask_path):
                os.remove(mask_path)
            vutils.save_image(maska, mask_path, nrow=10) 

            # imga = torch.tensor(imga)
            img_path = os.path.join("imga", "cos1", f"test_{i}.png")
            if os.path.exists(img_path):
                os.remove(img_path)
            vutils.save_image(imga, img_path, nrow=7)
...
#TODO3 step1-2: modify the path, MVTM parameters
    ...
    # print(MaskGit_CONFIGS['gamma_type'])
